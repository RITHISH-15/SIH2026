...
def render_hazard_cards(hazard_record: dict, unsafe_allow_html=True):
    """
...
"""
    st.markdown(textwrap.dedent(header_html, unsafe_allow_html=True).strip(, unsafe_allow_html=True), unsafe_allow_html=True, unsafe_allow_html=True)
...
def _render_single_hazard(key: str, meta: dict, data: dict):
    ...
    if risk_level == "HIGH" or prob_percent >= 65:
        ...
        badge_border = "#ef4444"
        badge_text = "#f87171"
        card_border = "rgba(239, 68, 68, 0.4, unsafe_allow_html=True)"
        advisory = "CRITICAL: Implement emergency readiness protocols."
    elif risk_level == "MEDIUM" or prob_percent >= 30:
        badge_bg = "rgba(245, 158, 11, 0.2)"
        badge_border = "#f59e0b"
        badge_text = "#fbbf24"
        card_border = "rgba(245, 158, 11, 0.4, unsafe_allow_html=True)"
        advisory = "ADVISORY: Monitor regional radar and sensors."
    else:
        badge_bg = "rgba(16, 185, 129, 0.2)"
        badge_border = "#10b981"
        badge_text = "#34d399"
        card_border = "rgba(16, 185, 129, 0.2, unsafe_allow_html=True)"
        advisory = "NOMINAL: Normal conditions expected."

    card_html = f"""
<div style="background: rgba(30, 41, 59, 0.85, unsafe_allow_html=True); border: 1px solid {card_border}; border-radius: 12px; padding: 14px; margin-bottom: 12px;">
...
"""
    st.markdown(textwrap.dedent(card_html, unsafe_allow_html=True).strip(, unsafe_allow_html=True), unsafe_allow_html=True, unsafe_allow_html=True)